...
MODULE Chemistry_Mod  ! chemistry_mod is the name of the chemistry module in SOSA. In MALTE-BOX the chemistry module is called CHEMISTRY.
  ...
  USE second_Global, ONLY : TEMP, J, NPHOT, M, N2, O2, H2O, RO2, cs_H2SO4, cs_HNO3
  ...
  !Public subroutines:
  PUBLIC :: KPP_SetUp, CHEMISTRY, PHOTOLYSIS_ARCA_INIT
...
CONTAINS
  ...
  SUBROUTINE PHOTOLYSIS_ARCA_INIT(INPUT_DIR,STATION)
      use FILECOUNT_MOD
      IMPLICIT NONE
      INTEGER :: I, ioi, JJ, cols
      CHARACTER(*), INTENT(IN) :: INPUT_DIR
      CHARACTER(*), INTENT(IN) :: STATION
      REAL :: ignore
      ...
      CS_QY = 0d0
      DO I=1,56
          OPEN(900, FILE = TRIM(ADJUSTL(INPUT_DIR))//'/Photolyse/MCM3_CS_QY/J'//TRIM(i2chr(I))//'.txt',STATUS = 'OLD', IOSTAT=ioi)
          if (ioi==0) THEN
              cols = COLCOUNT(900)
              DO JJ=1,75
  ...
  END SUBROUTINE PHOTOLYSIS_ARCA_INIT

  SUBROUTINE PHOTOLYSIS_ARCA(SAFN, T, J)

      IMPLICIT NONE

      INTEGER :: I, R
      REAL(dp), INTENT(IN)      :: SAFN(:)   ! spectral actinic flux
      REAL(dp), INTENT(INOUT)   :: J(:)      ! Photochemistry rates (some are empty)
      ...
      DO I=1,75
          ...
          R=2;  J(R) = J(R) &
                          + CS_QY(R,I,1) * EXP(CS_QY(R,I,2)/T) &
                          * CS_QY(R,I,3) * SAFN(I) * DL
          R=3;  J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=4;  J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=5;  J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=6;  J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=7;  J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          ! (T-independent) ACS, QY=1
          R=8;  J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,4) * SAFN(I) * DL
          ...
          R=13; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=14; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=15; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=16; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=17; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=18; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=19; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          ! Reaction J20 was previously missing
          R=20; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=21; J(R) = J(R) &
          ...
          R=22; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=23; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=24; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=31; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=32; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=33; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=34; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=35; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=41; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          ...
          R=53; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL

          R=54; J(R) = J(R) &
                     + CS_QY(R,I,1) * EXP(CS_QY(R,I,2) * (T-298)) &
                     * CS_QY(R,I,3) * SAFN(I) * DL

          R=55; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL
          R=56; J(R) = J(R) + CS_QY(R,I,1) * CS_QY(R,I,2) * SAFN(I) * DL

      ENDDO


  end SUBROUTINE PHOTOLYSIS_ARCA
  ...
  SUBROUTINE CHEMISTRY(CONS, &
    ...
    RO2_out, J_values_out, K_values_out, I_ACF_out,level)

    REAL(kind=dp), INTENT(inout)                    :: CONS(NSPEC)       ! Gas concentrations

    REAL(kind=dp), INTENT(in)                       :: time1_in          ! start of time step in KPP
    REAL(kind=dp), INTENT(in)                       :: time2_in          ! end of time step in KPP
    REAL(kind=dp), INTENT(in)                       :: temp_in           ! [K], temperature at this level of atmosphere
    REAL(kind=dp), INTENT(in)                       :: pres_in           ! [Pa], pressure at this level of atmosphere
    REAL(kind=dp), INTENT(in)                       :: zenith_deg_in     ! [deg], solar zenith angle
    REAL(kind=dp), INTENT(in)                       :: radiation_in      ! [-], incoming radiation already considering penetration trough the canopy
    REAL(kind=dp), INTENT(in)                       :: albedo_in         ! ground albedo from measurements (not sure for what wl)
    REAL(kind=dp), INTENT(in)                       :: cs_H2SO4_in       ! condensation sink of H2SO4
    REAL(kind=dp), INTENT(in)                       :: cs_HNO3_in        ! condensation sink of HNO3
    REAL(kind=dp), INTENT(in)                       :: M_in              ! [molec cm-3], number concentration of M (air)
    REAL(kind=dp), INTENT(in)                       :: N2_in             ! [molec cm-3], number concentration of N2
    REAL(kind=dp), INTENT(in)                       :: O2_in             ! [molec cm-3], number concentration of O2
    REAL(kind=dp), INTENT(in)                       :: H2O_in            ! [molec cm-3], number concentration of water vapor
...
!----------------------------------------------------------------------------------------------------------------------
    ...
    ! IF swr_is_af == .false.
    IF (.false.) THEN
        I_ACF(:) = radiation_in * swr_distribution(1:NWL)
    ELSE
        I_ACF = 1d-4/(6.62607004D-34*2.99792458D17) * [(i,i=280,650,5)] ! conversion from W/m² to photons/cm²/s
        I_ACF = I_ACF * radiation_in * swr_distribution(1:NWL) * (1 + 2 * albedo_in * COS(zenith_deg*3.1416/180) + albedo_in)
    END IF
    I_ACF_out = I_ACF

    CALL PHOTOLYSIS_ARCA(I_ACF, temp_in, J)
    WHERE (J_values_out > -1d0) J = J_values_out
    J_values_out = J
    ...
    CALL KPP_Proceed(CONS, time1_in, time2_in, level)
