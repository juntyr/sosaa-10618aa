...
MODULE psd_aerosol_dynamics

! USE second_precision, ONLY: dpa
USE psd_constants
! USE INPUT
USE PSD_scheme
! USE auxillaries
...
SUBROUTINE Condensation_apc(VAPOUR_PROP, conc_vap, dmass, dt_cond, d_dpar,d_vap, kelvin_effect)
    IMPLICIT NONE
    type(vapour_ambient), INTENT(INOUT)         :: VAPOUR_PROP  ! Properties of condensing vapours
...
SUBROUTINE deposition_velocity(d_p,ustar,Av,Au,Ad,V_chamber,T,p,E_field,dn_par, dt,k_dep)
    ...
    *ATAN((8.6-10.92*Sc**(-1D0/3D0))/(SQRT(3D0)*10.92*Sc**(-1D0/3D0)))
    ...
    ! END DO
    if (GTIME%printnow) print *, 'Particle loss rate (geom.mean):',  EXP(SUM(LOG(k_dep))/n_bins_par), &
                    '/s, ', EXP(SUM(LOG(k_dep))/n_bins_par)*3600d0, '/h'
...
END MODULE psd_aerosol_dynamics
